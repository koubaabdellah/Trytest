/**
 * @file
 * Contains the definition of the behaviour gravotech_dealers_locator_map.
 */
...
(function ($, Drupal) {
  ...
  /**
   * Attaches the Dealers Locator Behaviour.
   */
  Drupal.behaviors.gravotech_dealers_locator_map = {
    attach: function (context, settings) {

      var platform, pixelRatio, defaultLayers, behavior, ui, currentZoom, endZoom, ppi, clusterRatio;
      var viewArgs = new Array();
      var dataPoints = new Array();

      pixelRatio = window.devicePixelRatio || 1;

      function addInfoBulle(data) {
        map.addObject(group);
        var iconOptions = {
          // The icon's size in pixel:
          size: new H.math.Size(28 * pixelRatio, 30 * pixelRatio),
        };
        var icon = new H.map.Icon(settings.iconDealer, iconOptions);
        //For center to map on all store, we have to hide map marker and add them to a group
      ...
      function updateViews(elementValues, viewName) {
        jQuery.each(Drupal.views.instances, function (i, view) {
          if (view.settings.view_name == viewName) {
            view.settings.view_args = elementValues;
            view.settings = removeMatching(view.settings, RegExp('filters*'));
            for (var k in elementValues) {
              if (elementValues.hasOwnProperty(k)) {
                view.settings[k] = elementValues[k];
              }
            }
            view.settings['remove'] = 'remove';
            $('.js-view-dom-id-' + view.settings.view_dom_id).trigger('RefreshView');
          }
        });
      };

      //To remove old filter from settings array
      function removeMatching(viewSettings, regex) {
        var viewSetting;
        for (viewSetting in viewSettings) {
          if (regex.test(viewSetting)) {
            delete viewSettings[viewSetting];
          }
        }
        return viewSettings;
      }
      ...
      /**
       * CLICK/TAP event handler for our markers. That marker can
       * represent either a single photo or a cluster (group of photos)
      ...
      function onMarkerClick(e) {
        ...
        dealerPopIn(data.url);
      }

      function dealerPopIn(url) {
        var elementSettings = {
          progress: {
            type: 'throbber'
          },
        ...
        var currentTop = $('.ui-widget.ui-widget-content').css('top');
        $('.ui-widget.ui-widget-content').css('top',currentTop + $(window).scrollTop());
      }

      $('#map', context).once('myMapInit').each(function () {
        ...
        var data = settings.dealers;
        jQuery.each(data.dealerslist, function (index, store) {
          addInfoBulle(store);
        });
        map.addObject(group);
        ...
        map.addEventListener('mapviewchangeend', function (evt) {
          ...
          viewArgs['latitude[min]'] = BottomRight['lat'];
          viewArgs['latitude[max]'] = TopLeft['lat'];
          viewArgs['longitude[min]'] = TopLeft['lng'];
          viewArgs['longitude[max]'] = BottomRight['lng'];

          filters.forEach(function(filter) {
            viewArgs['filters['+filter+']'] = filter;
          });

          updateViews(viewArgs, 'dealer_list')
        });
      });

      $('.dealer-locator-search', context).once().on('submit', function (e) {
        e.preventDefault();
        ...
        var onResult = function (result) {
          ...
          // Center and zoom the map to the search location
          map.setCenter({
      ...
      $('.geolocate-me', context).once().on('click', function () {
      ...
      $('#views-exposed-form-dealer-list-block-map input[type=submit]').on('click', function (e) {
        var filtersChecked = $(this).parents('form:first').serializeArray();
        if ($(this).data('drupal-selector') == 'edit-reset') {
          e.preventDefault();
          $('#views-exposed-form-dealer-list-block-map select').prop('checked', false);
          filtersChecked = [];
          updateViews(viewArgs, 'dealer_list');
        }
        group.removeAll();
        filters = [];
        filtersChecked.forEach(function(filter) {
        ...
        markers.forEach(function (marker) {
          ...
          filtersChecked.forEach(function(filter) {
            printAll = false;
            if (marker.data.filters.includes(filter.value) !== false) {
              haveFilter = true;
            }
          });
          if(haveFilter === true  || printAll === true) {
            group.addObject(marker);
          }
        });
      });
      ...
      var options = {
        ...
        language: {
          inputTooShort: function (args) {
            ...
            return Drupal.t('Type more stuff', {}, {'context': 'Gravotech dealer locator'});
          },
          inputTooLong: function (args) {
            // args.maximum is the maximum allowed length
            // args.input is the user-typed text
            return Drupal.t('You typed too much', {}, {'context': 'Gravotech dealer locator'});
          },
          errorLoading: function () {
            return Drupal.t('Error loading results', {}, {'context': 'Gravotech dealer locator'});
          },
          loadingMore: function () {
            return Drupal.t('Loading more results', {}, {'context': 'Gravotech dealer locator'});
          },
          noResults: function () {
            return Drupal.t('No results found', {}, {'context': 'Gravotech dealer locator'});
          },
          searching: function () {
            return Drupal.t('Searching...', {}, {'context': 'Gravotech dealer locator'});
          },
        ...
        dropdownParent: $('.dealer-locator-search .form-item')
      };

      $('#search-location', context).select2(options).on('select2:select', function () {
        $('.dealer-locator-search').submit();
      });

      $('.dealer-locator', context).on('click', 'a.popup', function (e) {
        e.preventDefault();
        dealerPopIn($(this).attr('href'));
      });
    }
  };
})(jQuery, Drupal);
