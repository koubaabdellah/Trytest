...
function Bereken(iWelkeBerekening) {
    ...
    if (iWelkeBerekening == 0) { // Van aanslagnummer naar betalingskenmerk
        ...
        if (strMiddelcode == "w") {
            ...
            // Regex checkt de 2e en 5e positie. De tweede positie mag alleen 0, 6, 7 of 8 zijn als de vijfde positie 4 of 5 is. 
            var regexSlotcijfersVal1 = /^\d[^0678]\d{2}[45]/;
            // Regex checkt de 2e en 5e positie. De tweede positie mag geen nul zijn als de vijfde positie 6 is.
            var regexSlotcijfersVal2 = /^\d[0]\d{2}[6]/;

            if (regexSlotcijfersVal1.test(strSlotcijfers)) {
                alert('De combinatie van slotcijfers die u hebt ingevuld, is niet geldig. Als het 5e slotcijfer 4 of 5 is, dan moet het 2e slotcijfer een 0, 6, 7 of 8 zijn');
                return false;
            }
            if (regexSlotcijfersVal2.test(strSlotcijfers)) {
                alert('De combinatie van slotcijfers die u hebt ingevuld, is niet geldig. Als het 5e slotcijfer 6 is, dan mag het 2e slotcijfer namelijk niet 0 zijn.');
                return false;
            }
        ...
        // Van omzetbelastingnummer of loonheffingennummer naar betalingskenmerk
        if (sMiddelCode == "B") {
...
function doKeuzeHelpRestant(oThis) {
    ...
    else if ((strMiddelcode == "h") || (strMiddelcode == "n")) {
        return doHelp('divHelpRestantHN', oThis);
    }
    else if (strMiddelcode == "s") {
        return doHelp('divHelpRestantS', oThis);
    }
