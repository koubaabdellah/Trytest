...
(function($) {
  ...
  Drupal.behaviors.printButton = {
    attach: function(context) {
      // Only add a print button on a single factsheet or the print set page.
      if ($('body.node-type-factsheet').length > 0 || $('body.page-factsheets-print').length > 0 || $('body.page-factsheets-release-print').length > 0) {
        var printButtonWrapper = $('<div>');
        var printButton = $('<a href="#" class="print-button button">' + Drupal.t('Print') + '</a>');
        printButton.click(function(e) {
          window.print();
          e.preventDefault();
        });
        printButton.appendTo(printButtonWrapper);
        if ($('body.page-factsheets-print').length > 0 || $('body.page-factsheets-release-print').length > 0) {
          printButtonWrapper.insertAfter('#system-main > h1:first-of-type');
        }
        else {
          printButtonWrapper.insertAfter('.button');
        }
      }
    }
  };
