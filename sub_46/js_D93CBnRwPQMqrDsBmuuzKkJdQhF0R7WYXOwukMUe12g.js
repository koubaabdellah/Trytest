...
(function($) {
  /**
   * Behavior for print button on factsheet.
   */
  Drupal.behaviors.collapseFilters = {
    attach: function(context) {
      if (!$('.filter-toggle').length) {
        var filters = $('#factsheets-products, #factsheets-type, #factsheets-sources, #factsheets-topics');
        var currentSearch = $('#factsheets-current-search');
        var hideText = Drupal.t('Hide');
        var showText = Drupal.t('Show');
        var filtersToggleOuter = $('<div class="factsheets-filters clearfix">');
        var filtersToggle = $('<div class="factsheets-filters-toggler">');
        var filtersReset = $('#factsheets-reset-filters');
        // Only when no filter is applied.
        if (!filters.find('li.leaf.active').length) {
          filters.hide();
          filtersReset.hide();
        }
        var toggle = $('<a>', {
          'text': filters.is(':visible') ? hideText : showText,
          'class': filters.is(':visible') ? 'filter-toggle collapsed' : 'filter-toggle',
          'href': '#'
        }).bind('click', function (e) {
          currentSearch.slideToggle(499);
          filters.slideToggle(499);
          toggle.toggleClass('collapsed');
          toggle.text(filters.is(':visible') ? hideText : showText);
          filtersReset.toggle();
          e.preventDefault();
        ...
        filtersToggleOuter.prependTo('.content-top');
        filtersToggle.prependTo(filtersToggleOuter);
        $('#factsheets-search').appendTo(filtersToggleOuter);
        currentSearch.appendTo(filtersToggleOuter);
      }
    }
  };
