...
var setLang = function() {
  ...
  [...document.querySelectorAll(".projectdata")].forEach(el => {
    if(el.classList.contains(PPP.autocompleteLanguage)) {
      $(el).show();
    }
    else {
      $(el).hide();
...
$(document).ready(function() {
  ...
  // init search field
  $("#SearchAutoCompleteInput").clearField().bind("keypress", function(event) {
  ...
  $("#index-bar").find("a").on("click", function() {
    $(this).showIndex();
  ...
  showIndex: function () {
    ...
    if (indexCache[character]) {
      ...
      WHERE {\
        ...
        FILTER(regex(str(?prefLabel),'^"+ character +"','i') && lcase(lang(?prefLabel)) = lcase('"+ PPP.autocompleteLanguage +"')).\
      }\
      ORDER BY ?prefLabel";
      var data = {query: query, "content-type": "application/json"}
      $.getJSON(sparqlEndpointUrl, data, function (data) {
