...
//		this.routeParts.splice.apply(this, [0, 0].concat(FDUriRoute.is(route) ? route.routeParts : route.split('/')));
	...
	toString: function () {
		if (this.segments.length > 0) {
			return this.getBase() + (this.getPart(0) != 'login' || FD.isFrontend() ? this.identifier : '') + this.getRoute();
		} else {
			return this.getBase();
		}
	}
	...
	createFrontend: function () {
		var info = new FDUriRoute();
		info.setup.apply(info, arguments);
		info.identifier = this.FRONTEND;
		return info;
	},
	createBackend: function () {
		var info = new FDUriRoute();
		info.setup.apply(info, arguments);
		info.identifier = this.BACKEND;
		return info;
	},
	is: function (obj) {
...
//			this.log(['.getRouteInfo', this.routeInfo]);
		}
