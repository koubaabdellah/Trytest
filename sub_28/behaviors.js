/**
 * @file
 * Behaviors file for CKEditor.
 */

(function($) {

  /**
   * Create a margin on an inline image.
   */
  Drupal.behaviors.inlineImgMargin = {
    attach: function (context) {

      $('#block-system-main .content img, #block-system-main .content figure, .cke_editable img', context).once('image-margin').each(function() {
        var $image = $(this);
        var $editor = $image.parents('.cke_editable');

        if ($image.css('float') == 'left') {
          $image.addClass('position-left');
        ...
        if ($image.css('float') == 'right') {
          $image.addClass('position-right');

          if (!$editor) {
            $image.css('float', '');
          }
        }
        if ($image.css('width') && parseInt($image.css('width'))) {
          $image.attr('width', parseInt($image.css('width')));
          $image.css('width', '');
        }
        if ($image.css('height') && parseInt($image.css('height'))) {
          $image.attr('height', parseInt($image.css('height')));
          $image.css('height', '');
        }
      });
    }
  ...
  Drupal.behaviors.featureCKEditorChanges = {
    attach: function () {
        ...
        CKEDITOR.dialog.add('image2', function(editor) {
          ...
          var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
            ...
            lockResetHtml = new CKEDITOR.template(
              ...
              // Overwrite the original and set to true.
              var hasFileBrowser = !!(true),
              srcBoxChildren = [
              {
                id: 'src',
                type: 'text',
