...
//
function oc_text_dimensions (node, parent) {
    ...
    var text_width = Math.max(dimensions_title[0],dimensions_subtitle[0]);
    var text_height = Math.max(dimensions_title[1] + dimensions_subtitle[1]);
    node.dimensions_text = [text_width, text_height];
    ...
    if(parent !== undefined) {
        if(parent.oc_max_text_width === undefined) {
            parent.oc_max_text_width = {};
        }
        if(parent.oc_max_text_width[node.type] === undefined) {
            parent.oc_max_text_width[node.type] = oc_style.min_box_width;
        }

        // check node dimensions vs stored dimensions
        if (dimensions_title[0] > parent.oc_max_text_width[node.type])
            parent.oc_max_text_width[node.type] = dimensions_title[0];
        if (oc_style.use_images && oc_style.images_size[0] > oc_max_text_width)   // GG 110712 include space for optional image width
            parent.oc_max_text_width[node.type] = oc_style.images_size[0];
        if (dimensions_subtitle[0] > parent.oc_max_text_width[node.type])
            parent.oc_max_text_width[node.type] = dimensions_subtitle[0];
    }
    ...
    //calcualte the children's width
    for (var i = 0; i < node.children.length - (oc_IE); i++)
        oc_text_dimensions(node.children[i], node);

    //now equalize the widths
    for (var i = 0; i < node.children.length - (oc_IE); i++)
        node.children[i].dimensions_text[0] =  node.oc_max_text_width[node.children[i].type];
}
...
function oc_boundboxes_dimensions (node) {
    ...
    node.boundbox = [
        node.dimensions_text[0] + 2 * oc_style.inner_padding[0],
        oc_max_text_height + 2 * oc_style.inner_padding[1]
    ];

    // GG 110712 add vertical space for images within boxes
    if (oc_style.use_images && node.image !== undefined)
        node.boundbox[1] += oc_style.inner_padding[1] * 1 + oc_style.images_size[1];
...
function oc_draw_obj (node, parent, xoffset, yoffset) {
    ...
    var x0 = xc - node.dimensions_text[0]  / 2 - oc_style.inner_padding[0];
    var x1 = xc + node.dimensions_text[0]  / 2 + oc_style.inner_padding[0];
    // GG 110712 support for embeeded images
    if (oc_style.use_images && node.image !== undefined) {
        var y0 = yc - oc_max_text_height / 2 - oc_style.inner_padding[1] - oc_style.images_size[1] / 2 - oc_style.inner_padding[1] / 2;
        var y1 = yc + oc_max_text_height / 2 + oc_style.inner_padding[1] + oc_style.images_size[1] / 2 + oc_style.inner_padding[1] / 2;
    }
    else {
        var y0 = yc - oc_max_text_height / 2 - oc_style.inner_padding[1];
        var y1 = yc + oc_max_text_height / 2 + oc_style.inner_padding[1];
    }
    ...
    var title_ypos = y0 + oc_style.inner_padding[1]
    ...
    if (node.subtitle !== undefined) {
        var subtitle_ypos = y1 - oc_style.inner_padding[1]
    ...
    if (oc_style.use_images && node.image !== undefined) {
        var image = oc_paper.image(oc_style.images_base_url + node.image,
            xc - oc_style.images_size[0] / 2, y1 - oc_style.images_size[1] - oc_style.inner_padding[0],
            oc_style.images_size[0], oc_style.images_size[1]);
    }

    // draw line connectors
    if (parent != null) {
        var pxc = xc - node.xoffset - node.deltacorner[0] + parent.xoffset + ( 0 - 0) + ((parent.dimensions_text[0] - node.dimensions_text[0])/2);
        var pyc = yc -            0 - node.deltacorner[1] +              0 + (parent.deltacenter[1] - node.deltacenter[1]);
        var px0 = pxc - parent.dimensions_text[0]  / 2 - oc_style.inner_padding[0];
        var px1 = pxc + parent.dimensions_text[0]  / 2 + oc_style.inner_padding[0];
        var py0 = pyc - oc_max_text_height / 2 - oc_style.inner_padding[1];
        var py1 = pyc + oc_max_text_height / 2 + oc_style.inner_padding[1];
        var line1, line2;
        ...
        line1.attr({'stroke': oc_style.line_color, 'stroke-width':1});
        line2.attr({'stroke': oc_style.line_color, 'stroke-width':1});
    }
