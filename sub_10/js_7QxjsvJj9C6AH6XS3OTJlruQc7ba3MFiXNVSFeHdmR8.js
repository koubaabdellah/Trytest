...
(function ($, Drupal, drupalSettings) {
  'use strict';
  Drupal.behaviors.gravotech_paragraph_component_news_events = {
    attach: function (context, settings) {
      $(".news-events-wrapper", context).each(function(index) {
        var $this = $(this);
        $this.find('.swiper-news-events').addClass("swiper-news-events-" + index);
        $this.find(".swiper-button-next").addClass("news-events-button-next-" + index);
        $this.find(".swiper-button-prev").addClass("news-events-button-prev-" + index);
        var swiper = new Swiper(".swiper-news-events-" + index, {
          effect: 'slide',
          direction: 'horizontal',
          slidesPerView: 3,
          spaceBetween: 30,
          ...
          navigation: {
            nextEl: '.news-events-button-next-'+ index,
            prevEl: '.news-events-button-prev-'+ index,
          },
...
(function ($, Drupal, drupalSettings) {
  'use strict';
  Drupal.behaviors.gravotech_homepage_solution  = {
    attach: function (context, settings) {
      // select drop down for solution section
      let labels = $('.js-form-type-select label', context);
      let selects = $('#views-exposed-form-component-solutions-homepage-block .form-select', context);

      $(selects).each(function(index) {
        $(this).select2({
          placeholder: labels[index].innerHTML,
          width: 'resolve',
          closeOnSelect: false
        });
      });
